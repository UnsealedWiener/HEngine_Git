...
Texture2D Textures[350]:    register(t0, space2);
...
[shader("closesthit")]
void Hit0(inout Payload payload : SV_Payload, in MyAttributes attr)
{
	...
	float3 emissive = SampleTextureByFloat(Textures[instanceData.matIndex0 + 6], textureCoord.xy, mipmapLevel);
	...
	float3 color = directLighting + pointLighting+ spotLighting+  ambientLighting* ao + emissive;
