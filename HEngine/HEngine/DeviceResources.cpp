...
DeviceResources* DX::DeviceResources::GetInstance()
{
	static DeviceResources deviceResources;
	return &deviceResources;
}

void DX::DeviceResources::Initialize(DXGI_FORMAT backBufferFormat, DXGI_FORMAT depthBufferFormat, UINT backBufferCount, D3D_FEATURE_LEVEL minFeatureLevel, unsigned int flags)
{
	m_backBufferFormat = backBufferFormat;
	m_depthBufferFormat = depthBufferFormat;
	m_backBufferCount = backBufferCount;
...
void DeviceResources::CreateDeviceResources()
{
	...
	//그래픽 메모리를 보기 위한 DXGIAdapter3를 가져온다.
	LUID luid = m_d3dDevice->GetAdapterLuid();
	m_dxgiFactory->EnumAdapterByLuid(luid, IID_PPV_ARGS(&m_dxgiAdapter));
...
void DX::DeviceResources::SetScreenSizeViewport()const
{
	m_commandList->RSSetViewports(1, &m_screenViewport);
	m_commandList->RSSetScissorRects(1, &m_scissorRect);
}

void DX::DeviceResources::SetShadowMapViewport()const 
{
	D3D12_VIEWPORT  shadowViewport;
	
	shadowViewport.TopLeftX = shadowViewport.TopLeftY = 0.f;
	shadowViewport.Width = static_cast<float>(2048);
	shadowViewport.Height = static_cast<float>(2048);
	shadowViewport.MinDepth = D3D12_MIN_DEPTH;
	shadowViewport.MaxDepth = D3D12_MAX_DEPTH;

	D3D12_RECT	scossorRect;

	scossorRect.left = scossorRect.top = 0;
	scossorRect.right = static_cast<LONG>(2048);
	scossorRect.bottom = static_cast<LONG>(2048);

	m_commandList->RSSetViewports(1, &shadowViewport);
	m_commandList->RSSetScissorRects(1, &scossorRect);
}

void DX::DeviceResources::SetUiViewPort() const
{
	D3D12_VIEWPORT  viewport = m_screenViewport;
	viewport.MaxDepth = 0.01;
...
bool DX::DeviceResources::CheckRaytracingSupport()
{
	D3D12_FEATURE_DATA_D3D12_OPTIONS5 options5 = {};
	DX::ThrowIfFailed(m_d3dDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS5,
...
void DX::DeviceResources::CreateRaytracingInterface()
{
	DX::ThrowIfFailed(m_d3dDevice->QueryInterface(IID_PPV_ARGS(&m_dxrDevice)));
	DX::ThrowIfFailed(m_commandList->QueryInterface(IID_PPV_ARGS(&m_dxrCommandList)));
}

void DX::DeviceResources::CheckMsaaSupported()
{
	for (m_sampleCount = c_targetSampleCount; m_sampleCount > 1; m_sampleCount--)
	{
		...
		if (0 > m_d3dDevice->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &levels, sizeof(levels)))
			continue;

		m_sampleQuality = levels.NumQualityLevels;
		if (levels.NumQualityLevels > 0)
			break;
	}

	if (m_sampleCount < 2)
		m_bMsaaSupported = false;
	else
		m_bMsaaSupported = true;
}
